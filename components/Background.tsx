...
type PacketCategory = 'advert' | 'message' | 'route' | 'control';
...
const COLORS: Record<'light' | 'dark', Record<PacketCategory, string>> = {
  light: {
    advert: '#07877f',
    message: '#2563eb',
    route: '#7c3aed',
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      for (const trail of trails) {
        ...
        context.globalAlpha = Math.min(0.78, 0.5 * trail.intensity) * fade;
        context.strokeStyle = COLORS[theme][trail.category];
        context.lineWidth = Math.min(3.2, 1.8 * trail.intensity);
        context.lineCap = 'round';
        context.lineJoin = 'round';
        context.shadowColor = COLORS[theme][trail.category];
        context.shadowBlur = 10;
        context.stroke();

        const headPoint = pointOnCurve(trail, head);
        context.globalAlpha = Math.min(0.85, 0.58 * trail.intensity) * fade;
        context.beginPath();
        context.arc(
          headPoint.x * width,
          headPoint.y * height,
        ...
        context.fillStyle = COLORS[theme][trail.category];
        context.fill();
      }
    ...
    const connect = () => {
      ...
      nextSocket.addEventListener('message', (event) => {
        ...
        try {
          ...
          // Ignore malformed or non-JSON analyzer messages.
        }
      });
    ...
    const handleVisibility = () => {
      ...
      if (!shouldConnect) stopLiveActivity();
      else {
        lastLivePacketAt = performance.now();
        connect();
        void loadReplayBuffer();
    ...
    if (!reducedMotion.matches) {
      ...
      const isDemo = new URLSearchParams(window.location.search).has(
        'mesh-demo'
      );
